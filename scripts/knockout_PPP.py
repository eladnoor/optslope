...
def draw_PPP(title, target_reaction, knockins, carbon_sources, knockouts,
             rbc_upper_bound=100):
    for ki in knockins:
        wt_model = init_wt_model('core', {}, BM_lower_bound=0.1)
        knockin_reactions(wt_model, ki, 0, 1000)
        knockin_reactions(wt_model, 'EDD,EDA', 0, 1000)
        knockin_reactions(wt_model, 'EX_xu5p_D,EX_r5p,EX_dhap,EX_2pg,EX_e4p,EX_6pgc', 0, 0)
        ...
        for y, carbon_source in enumerate(carbon_sources):
            for x, knockout in enumerate(knockouts):
                ...
                wt_PPP, wt_slope = get_PPP(temp_model, target_reaction)
                ...
                knockout_reactions(temp_model, knockout)
                ko_PPP, slope = get_PPP(temp_model, target_reaction)
                if slope is None:
